...
export default function LoginModal({ isOpen, onClose, onSuccess }: LoginModalProps) {
  ...
  const handlePhoneSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const checkRes = await fetch('/bdapps-auth/api/check_subscription', {
        method: 'POST',
      ...
      const res = await fetch('/bdapps-auth/api/send_otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams({ user_mobile: phone }).toString()
      });
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/bdapps-auth/api/verify_otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    ...
    <div className={styles.overlay}>
      <div className={`${styles.modal} animate-scale-up glass-panel`}>
        ...
        <div className={styles.header}>
          ...
          <p>{step === 'phone' 
            ? 'সাবস্ক্রিপশন করতে আপনার রবি/এয়ারটেল নম্বর দিন। চার্জ: ২.৭৮ টাকা/দিন (Vat+SC+SD সহ)' 
